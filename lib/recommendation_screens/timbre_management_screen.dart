...
class _TimbreManagementScreenState extends State<TimbreManagementScreen> {
  Future<void> _deleteTimbre(int timbreId) async {
    final response = await http.delete(
      Uri.parse('http://13.125.27.204:8080/timbre/$timbreId'),
    );
  ...
  Future<void> _updateTimbre(int timbreId, String newTimbreName) async {
    final response = await http.patch(
      Uri.parse('http://13.125.27.204:8080/timbre/$timbreId?timbre_name=$newTimbreName'),
    );
                            ...
                            TextButton(
                              ...
                              onPressed: () {
                                ...
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (context) => TimbreTestPage(cameFrom: 'timbre_management_screen'),
                                  ),
                                );
                              },
                            ),
              ...
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      title: Text(timbre['timbreName'], style: TextStyle(fontSize: 18)),
                      trailing: Row(
                                  ...
                                  return AlertDialog(
                                    ...
                                    actions: [
                                      ...
                                      TextButton(
                                        child: Text('수정'),
                                        onPressed: () {
                                          _updateTimbre(timbre['timbreId'], newTimbreName);
                                          Navigator.pop(context);
                                        },
                                      ),
                                    ],
